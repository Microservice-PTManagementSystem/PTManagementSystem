...
import { useRouter } from 'next/navigation';  // Router kullanarak yönlendirme işlemi yapacağız
//import { Button } from "../components/Button"; // Button component'ini dışarıdan alıyoruz
...
function LogInForm() {
  ...
  useEffect(() => {
    if (session) {
      router.push("/home"); // Oturum açıldıysa home sayfasına yönlendir
    }
  }, [session]);
  ...
  const handleSubmit = (e) => {
    ...
    // Burada Keycloak ile kimlik doğrulama yapılacak
  };

  const handleSignInWithKeycloak = async () => {
    await signIn("keycloak",{callbackUrl:"/home"}); // Keycloak ile giriş yapma işlemi
    console.log("-------------------GİRİŞ İŞLEMİ BAŞARILI---------------");
  };

  const handleSignInWithGoogle = () => {
    signIn("google"); // Google ile giriş yapma işlemi
  };

  const handleSignOut = async () => {
    await signOut({ redirect: false }); // Çıkış yapma işlemi
    router.push('/signup'); // Çıkış yaptıktan sonra signin sayfasına yönlendir
    ...
    <div className="flex w-full max-w-4xl overflow-hidden rounded-3xl bg-white shadow-2xl">
      ...
      <div className="relative hidden w-1/2 bg-black md:block">
        <Image
          ...
          fill 
          style={{ objectFit: "cover" }} // Resmin boyutları koruyarak alanı kaplamasını sağlar
          className="z-0"
        />
      ...
      <div className="w-full p-8 md:w-1/2 md:p-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Eğer kullanıcı oturum açmışsa çıkış yapma butonunu göster */}
          {session && (
            <Button
              type="button"
