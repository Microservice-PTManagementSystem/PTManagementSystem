...
export default function Home() {
  ...
  /*const handleSignOut = async () => {
    ...
    router.push('/'); // Çıkış yaptıktan sonra signin sayfasına yönlendir
    console.log("--------------ÇIKIŞ YAPILDI---------------");
  };*/
  /*const handleSignOut = () => {
    // 1) NextAuth çerezlerini sil
    signOut({ redirect: false }).then(() => {
      // 2) Keycloak oturumunu kapat
      const base   = "http://localhost:8080";
      const realm  = "ptmanagement";
      const client = "nextjs-app";
  
      const redirect = encodeURIComponent("http://localhost:3000");
      const idToken  = encodeURIComponent(session.idToken);   // callback’te eklemiştik
  ...
  const handleSignOut = () => {
    // 1) NextAuth çerezleri silinsin
    signOut({ redirect: false }).then(() => {
      // 2) Keycloak SSO oturumu kapansın
      const kcLogout =
      "http://localhost:8080/realms/ptmanagement/protocol/openid-connect/logout" +
 ...
 //normal register yaprtığımda kullanıcı ekleniyor session kısmına sonra yeni register denediğimde o kullanıcı aktif oluyor.(arada boşa signin de var).
 //kullanıcıdan çıkış yapmak istediğimde de invalid uri hatası alıyorum 
  return (
  ...
  {/* Resmin arkasına yerleştirilen koyu şeffaf katman */}
  <div className="absolute inset-0 bg-black/70 z-10"></div>
  
  {/* Resim */}
  <div className="relative h-[500px]">
    <Image
      src="/gym_background.jpg"  // Public klasörüne yüklediğiniz resim
      alt="Gym background"
      ...
      className="z-0"  // Resmin z-index'ini en düşük yapıyoruz, böylece metinler ön planda olur
    />
  </div>
